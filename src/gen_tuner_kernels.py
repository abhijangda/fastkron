...
class KernelConfig:  
  ...
  def hostFuncDecl(self):
    return f"void {self.hostFuncName()}(KernelParams<{self.fused_kernels}> params, FusedParams<{self.fused_kernels}> fusedParams, DistributedParams distParams, EpilogueParams<float> epilogueParams, dim3 grid, dim3 block, cudaStream_t stream)"

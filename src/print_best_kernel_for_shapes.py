...
def tune(mmtype, ms, n, p, q, opX, opF, fuse, backend, elemtype):
  ...
  # for m in ms:
    o = run_command("FASTKRON_LOG=DEBUG " + ("OMP_NUM_THREADS=64 taskset -c 0-64" if backend == "x86" else "") + f'../build/tests/benchmarks/benchmark_{backend} -m {m} -n {n} -p {p} -q {q} -r {10} -w {10} -t {elemtype} --tune --backend {backend} {"--fuse" if fuse else ""} -a 1 -b 0 --gemmtype {mmtype}')
    ...
    for q in [2,4,8,16,32, 64, 128]:
      ...
      for n in range(1,13):
        for m in [2,4,16,64,256]:
          ...
          #   continue
          if p <= 32:
            tune(args.mmtype, m, n, p, q, args.opX, args.opF, False, args.backend, elemType)
          tune(args.mmtype, m, n, p, q, args.opX, args.opF, True, args.backend, elemType)
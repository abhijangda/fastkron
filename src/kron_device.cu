// #define C_IN_SHMEM
template<uint MaxKronCols, uint KP_N_TILE> __device__ uint get_tile_k() {return blockIdx.y/DIVUP(MaxKronCols, KP_N_TILE);}
template<uint MaxKronCols, uint KP_N_TILE> __device__ uint get_external_tile_kp_n() {return blockIdx.y%DIVUP(MaxKronCols, KP_N_TILE);}
...
template<typename ElemT, typename VecT, uint NumThreads, uint N_COARSE_TB, uint TileRowsA, uint MaxColsA, uint MaxKronCols, uint MaxKronRows, uint KP_N_TILE_, uint K_EQUALS_VAR, uint KPK_EQUALS_VAR>
__global__ void kronGemmKernel(const uint RowsC,    const uint ColsC,   const uint ColsA,
                               ...
                               const uint kp_idx) {
  const uint KP_N_TILE = MIN(KP_N_TILE_, MaxKronCols);
  const uint NUM_KP_N_TILES = MaxKronCols/KP_N_TILE;
  const uint INTERNAL_KP_N_TILE = MIN(128, KP_N_TILE);
  const uint EXTERNAL_KP_K_TILE = MIN(EXTERNAL_KP_K_TILE_, MaxKronRows);
  const uint INTERNAL_KP_K_TILE = MIN(32, EXTERNAL_KP_K_TILE);

  // printf("MaxColsA %d MaxKronCols %d MaxKronRows %d KP_N_TILE_ %d\n", MaxColsA, MaxKronCols, MaxKronRows, KP_N_TILE_);
  const uint TileColA = MaxColsA/(MaxKronRows/INTERNAL_KP_K_TILE);
  ...
  if (KP_N_TILE == MaxKronCols && INTERNAL_KP_N_TILE == MaxKronCols && INTERNAL_KP_K_TILE == MaxKronRows) {
  ...
  for (uint start_row = blockIdx.x * TileRowsA; start_row < gridDim.x * TileRowsA * N_COARSE_TB; 
       start_row += gridDim.x * TileRowsA) {
  ...
  for (uint a_col_start  = a_col_start_ ; a_col_start  < MaxColsA/MaxKronRows;
    ...
    #pragma unroll
    for(uint tile_row = 0; tile_row < TileRowsA; tile_row++) {
      #pragma unroll
    ...
    for (uint internal_tile_kp_k = 0; internal_tile_kp_k < EXTERNAL_KP_K_TILE; internal_tile_kp_k += INTERNAL_KP_K_TILE) {
      for (uint aRow = 0; aRow < TileRowsA; aRow += 1) {
        for (uint a_col = tid*VecTNumElems; a_col < TileColA; a_col += blockDim.x*VecTNumElems) {
          uint tile_k = get_tile_k<MaxKronCols, KP_N_TILE>();
          VecT a;
          if (INTERNAL_KP_K_TILE == MaxKronRows) {
      ...
      for (uint internal_tile_kp_n = 0; internal_tile_kp_n < KP_N_TILE; internal_tile_kp_n += INTERNAL_KP_N_TILE) {
        if (!(KP_N_TILE == MaxKronCols && INTERNAL_KP_N_TILE == MaxKronCols && INTERNAL_KP_K_TILE == MaxKronRows)) {
          ...
          for (uint swid = tid/(INTERNAL_KP_N_TILE/ldSize); swid < INTERNAL_KP_K_TILE; swid += blockDim.x/(INTERNAL_KP_N_TILE/ldSize)) {
            uint external_tile_kp_n = get_external_tile_kp_n<MaxKronCols, KP_N_TILE>();
    ...
    #pragma unroll 
    for (int aRow = 0; aRow < TileRowsA; aRow++) {
      #pragma unroll 
      for (uint reg_j = 0; reg_j < CRegCols; reg_j++) {
        if (CRegRows % 4 == 0) {
          for (uint reg_i = 0; reg_i < CRegRows; reg_i += 4) {          
            ...
            if (KP_N_TILE != MaxKronCols) {
              uint external_tile_kp_n = get_external_tile_kp_n<MaxKronCols, KP_N_TILE>();
              cCol += external_tile_kp_n*(colsA/(MaxKronCols/KP_N_TILE)); 
            }

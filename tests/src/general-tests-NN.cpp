...
#define GENERAL_TEST_NN(M, MinFacs, MaxFacs, P, Q, Type, Tune) \
  ...
  for (uint Facs = MinFacs; Facs <= MaxFacs; Facs++) {\
    ...
    result = result and run<Type>(M, N, K, Facs, KP_MAT_N, KP_MAT_K, fastKronOp_N, fastKronOp_N, 1,1,1,(Type)2.0f, (Type)3.0f, 1, 0, false, 1, 1, 1, 1, true, true, Tune, getTestBackend(), false);\
    if (!result) abort();\
  }\
  EXPECT_TRUE(result);\
}

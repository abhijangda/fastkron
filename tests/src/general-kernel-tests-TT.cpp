...
#define FUSION_TEST(M, MinFacs, MaxFacs, P, Q, Type) \
  ...
  for (uint Facs = MinFacs; Facs <= MaxFacs; Facs++) {\
    ...
    result = result and run<Type>(M, N, K, Facs, KP_MAT_N, KP_MAT_K, fastKronOp_N, fastKronOp_T, 1, 0, false, 1, 1, 1, 1, true, true, true, getTestBackend(), false);\
    if (!result) abort();\
  }\
  EXPECT_TRUE(result);\
}
...
FUSION_TEST(M_, 2, 6, 8, 8, float);
FUSION_TEST(M_, 2, 5, 12, 12, float);
FUSION_TEST(M_, 2, 5, 16, 16, float);

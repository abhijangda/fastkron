...
#define MULTI_GPU_TUNER_TEST(M, Facs, FacSize, GM, GK, KronBatch, Type) \
  ...
  bool b = run<Type>(M, N, K, Facs, KP_MAT_N, KP_MAT_K, fastKronOp_N, fastKronOp_N, 1.0f, 0.0f, 0, 0, false, GM, GK, GM*GK, KronBatch, true, true, true, fastKronBackend_CUDA, false);\
  EXPECT_TRUE(b);\
}

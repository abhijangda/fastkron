...
x = torch.ones((M, reduce((lambda a, b: a * b), Ps)), dtype=torch.float32)
y = torch.zeros((M, reduce((lambda a, b: a * b), Qs)), dtype=torch.float32)
fs = [torch.ones((Ps[0], Qs[0]), dtype=torch.float32) for i in range(0, N)]

rs, ts = fastKron.gekmmSizes(x, fs)

t1 = torch.zeros(rs, dtype=torch.float32)
